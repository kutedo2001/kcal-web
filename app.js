...
const PLAN = window.NUTRI_PLAN;
const OCR = window.NUTRI_OCR;
...
function blankProfile(name) {
  return {
    ...
    mode: 'dyn', baseMult: 1.22, adapt: true, adaptFactor: 1,
    carry: true, carryCap: 400, carryReset: 'week',
    mealsPerDay: 4, ocrEndpoint: '',
    days: {}, recipes: [], customs: [], recent: {}, pantry: [], plan: null
  };
}
...
function load() {
  ...
  P.pantry ||= []; P.mealsPerDay ||= 4; P.ocrEndpoint ??= '';
}
let saveT;
...
function renderPantryExtras() {
  ...
  if (!P.recipes.length) rl.innerHTML = '<div class="list"><div class="empty">Todavía no has guardado ningún plato.<br>Apunta una comida en Hoy y pulsa «Guardar como plato».</div></div>';
    ...
    P.recipes.forEach(rec => {
      ...
      b.oncontextmenu = ev => { ev.preventDefault(); if (confirm(`¿Borrar el plato «${rec.name}»?`)) { P.recipes = P.recipes.filter(x => x !== rec); save(); renderPantry(); } };
      list.appendChild(b);
    });
    rl.appendChild(list);
  }
...
function renderSettings() {
  ...
  $('#pOcrEndpoint').value = P.ocrEndpoint || '';
  $('#pRate').value = P.rate;
  ...
  $('#modeNote').innerHTML = P.mode === 'dyn'
    ? 'El objetivo cambia cada día: base sin moverte + lo que gastas con los pasos y el entreno. Es el que da más información. El multiplicador de abajo NO incluye tu actividad: solo cubre la digestión y el movimiento que no son pasos ni gimnasio (1,20-1,25).'
...
/* ================= Navegación ================= */
const VIEWS = ['Today', 'Plan', 'Pantry', 'Progress', 'Settings'];
function go(v) {
  currentView = v;
  VIEWS.forEach(x => $('#view' + x).classList.toggle('hide', x !== v));
  $$('#tabs button').forEach(b => b.setAttribute('aria-current', b.dataset.view === v));
  const titles = { Today: niceDate(viewDate), Progress: 'Progreso', Pantry: 'Despensa', Plan: 'Plan', Settings: 'Ajustes' };
...
function openSearch(opts) {
  pickCallback = (opts && opts.pick) || null;
  searchMeal = guessMeal();
  $('#segMeal').parentElement && ($('#segMeal').style.display = pickCallback ? 'none' : '');
  $('#sheetSearch .head h3').textContent = pickCallback ? 'Elegir alimento' : 'Añadir';
...
function foodList(arr) {
  ...
  arr.forEach(f => {
    ...
    b.onclick = () => pickCallback ? pickCallback(f) : openPortion({ food: f });
    list.appendChild(b);
  });
  return list;
}
...
async function onScan(code) {
  ...
  if (pickCallback) return pickCallback(f);
  openPortion({ food: f });
}
...
$('#btnPortionSave').onclick = () => {
  ...
  if (poState.editing) Object.assign(poState.editing, entry);
    ...
    if (!inPantry(f.id)) addToPantry(f, true);
  }
...
/* ---------- Alimento a mano ---------- */
let pendingBarcode = null, customCallback = null;
function openCustom(barcode, prefill, cb) {
  pendingBarcode = barcode || null;
  customCallback = cb || null;
  $('#cuName').value = prefill ? String(prefill).toLowerCase().replace(/^\w/, c => c.toUpperCase()) : ''; $('#cuKcal').value = ''; $('#cuP').value = ''; $('#cuC').value = ''; $('#cuF').value = ''; $('#cuUnit').value = '';
...
$('#btnCustomSave').onclick = () => {
  ...
  if (customCallback) { const cb = customCallback; customCallback = null; return cb(f); }
  if (pickCallback) return pickCallback(f);
  addToPantry(f, true); save();
  openPortion({ food: f });
};
...
$('#pOcrEndpoint').addEventListener('change', e => { P.ocrEndpoint = e.target.value.trim(); save(); });
...
$$('#segMode button').forEach(b => b.onclick = () => {
  P.mode = b.dataset.v;
  P.baseMult = P.mode === 'dyn' ? 1.22 : 1.45;
  save(); renderSettings();
});
...
function pantryFoods() {                       // solo lo disponible, para el plan
  return P.pantry.filter(x => x.have).map(x => x.food);
}
function inPantry(id) { return P.pantry.find(x => x.food.id === id); }

function addToPantry(food, have) {
  const ex = inPantry(food.id);
  if (ex) { ex.have = have !== false; ex.n = (ex.n || 0) + 1; return ex; }
  const it = { id: uid(), food, have: have !== false, n: 1, added: todayISO() };
  P.pantry.unshift(it);
  return it;
}

function renderPantry() {
  const box = $('#pantryList'); box.innerHTML = '';
  const have = P.pantry.filter(x => x.have).length;
  $('#pantryCount').textContent = P.pantry.length ? `${have} de ${P.pantry.length} disponibles` : '';
  seg('#segPantryFilter', pantryFilter);

  const list = P.pantry.filter(x =>
    pantryFilter === 'all' || (pantryFilter === 'have' ? x.have : !x.have));

  if (!P.pantry.length) {
    box.innerHTML = '<div class="list"><div class="empty">La despensa está vacía.<br>Escanea un ticket o añade alimentos a mano.</div></div>';
  } else if (!list.length) {
    box.innerHTML = '<div class="list"><div class="empty">Nada en este filtro.</div></div>';
  } else {
    const byCat = {};
    list.forEach(x => { const c = PLAN.classify(x.food); (byCat[c] ||= []).push(x); });
    const order = ['prot', 'lacteo', 'carb', 'verd', 'fruta', 'grasa', 'otro'];
    const names = { prot: 'Proteína', lacteo: 'Lácteos', carb: 'Hidratos', verd: 'Verdura', fruta: 'Fruta', grasa: 'Grasas', otro: 'Otros' };
    order.filter(c => byCat[c]).forEach(c => {
      box.appendChild(el('div', 'grouphead', `<span>${names[c]}</span><span class="tiny">${byCat[c].length}</span>`));
      const l = el('div', 'list');
      byCat[c].forEach(x => {
        const row = el('div', 'item');
        row.innerHTML = `<div class="grow"><div class="t">${esc(x.food.n)}</div>
          <div class="s">${r0(x.food.kcal)} kcal · P${r1(x.food.p)} C${r1(x.food.c)} G${r1(x.food.f)}</div></div>`;
        const sw = el('button', 'sw', '<i></i>');
        sw.setAttribute('aria-pressed', x.have);
        sw.onclick = ev => { ev.stopPropagation(); x.have = !x.have; save(); renderPantry(); };
        row.appendChild(sw);
        row.onclick = () => openPortion({ food: x.food });
        row.oncontextmenu = ev => {
          ev.preventDefault();
          if (confirm(`¿Quitar «${x.food.n}» de la despensa?`)) { P.pantry = P.pantry.filter(y => y !== x); save(); renderPantry(); }
        };
        l.appendChild(row);
      });
      box.appendChild(l);
...
$$('#segPantryFilter button').forEach(b => b.onclick = () => { pantryFilter = b.dataset.v; renderPantry(); });
...
function openTicket() {
  ticketRows = [];
  $('#ticketStart').classList.remove('hide');
  $('#ticketProgress').classList.add('hide');
  $('#ticketReview').innerHTML = '';
  $('#btnTicketAdd').classList.add('hide');
  $('#ticketEngine').textContent = P.ocrEndpoint
    ? 'Lectura con tu servicio de visión.'
    : 'Lectura en el propio móvil. La primera vez descarga unos 3 MB de idioma.';
  openSheet('#sheetTicket');
}

$('#btnScanTicket').onclick = openTicket;
$('#btnTicketPhoto').onclick = () => $('#fileTicket').click();
$('#fileTicket').addEventListener('change', async e => {
  const f = e.target.files[0]; e.target.value = '';
  if (!f) return;
  $('#ticketStart').classList.add('hide');
  $('#ticketProgress').classList.remove('hide');
  const prog = (p, msg) => { $('#ticketBar').style.width = Math.round(p * 100) + '%'; if (msg) $('#ticketStatus').textContent = msg; };
  try {
    const lines = P.ocrEndpoint
      ? await OCR.readRemote(f, P.ocrEndpoint, prog)
      : await OCR.readLocal(f, prog);
    ticketRows = OCR.reviewList(lines, P.customs);
    $('#ticketProgress').classList.add('hide');
    if (!ticketRows.length) {
      $('#ticketReview').innerHTML = '<div class="list"><div class="empty">No he sacado nada legible.<br>Prueba con más luz o el ticket más plano.</div></div>';
      $('#ticketStart').classList.remove('hide');
      return;
    }
    renderTicketReview();
    ...
    $('#ticketProgress').classList.add('hide');
    $('#ticketStart').classList.remove('hide');
    $('#ticketEngine').textContent = 'Error: ' + err.message;
  }
});

function renderTicketReview() {
  const box = $('#ticketReview'); box.innerHTML = '';
  const n = ticketRows.filter(r => r.state !== 'fuera' && r.chosen).length;
  $('#btnTicketAdd').classList.toggle('hide', !n);
  $('#btnTicketAdd').textContent = n ? `Añadir ${n}` : 'Añadir';

  box.appendChild(el('div', 'grouphead',
    `<span>Revisa antes de añadir</span><span class="tiny">${ticketRows.length} líneas</span>`));

  ticketRows.forEach((r, i) => {
    const c = el('div', 'card');
    const chip = r.state === 'fuera' ? '<span class="pill">descartada</span>'
      : r.chosen && r.conf >= 0.6 ? '<span class="pill ok">detectado</span>'
        : r.chosen ? '<span class="pill warn">revisa</span>'
          : '<span class="pill bad">sin identificar</span>';

    c.innerHTML = `<div class="row between" style="margin-bottom:8px">
        <div class="grow tiny" style="font-family:ui-monospace,monospace">${esc(r.raw.slice(0, 46))}</div>${chip}</div>`;

    const pickBtn = el('button', 'btn');
    pickBtn.style.marginBottom = '8px';
    pickBtn.textContent = r.chosen ? r.chosen.n : 'Elegir alimento';
    if (r.chosen) pickBtn.classList.add('pri');
    pickBtn.onclick = () => openSearch({
      pick: f => { r.chosen = f; r.state = 'ok'; closeSheet('#sheetSearch'); renderTicketReview(); }
    });
    c.appendChild(pickBtn);

    if (r.cand.length > 1) {
      const alt = el('div', 'seg');
      r.cand.slice(0, 3).forEach(x => {
        const b = el('button', null, x.food.n);
        b.setAttribute('aria-pressed', r.chosen && r.chosen.id === x.food.id);
        b.onclick = () => { r.chosen = x.food; r.state = 'ok'; renderTicketReview(); };
        alt.appendChild(b);
      });
      c.appendChild(alt);
    }
    ...
    const bNew = el('button', 'btn sm', 'Crear a mano');
    bNew.style.flex = '1';
    bNew.onclick = () => { closeSheet('#sheetTicket'); openCustom(null, r.name, f => { r.chosen = f; r.state = 'ok'; openSheet('#sheetTicket'); renderTicketReview(); }); };
    const bOut = el('button', 'btn sm ' + (r.state === 'fuera' ? 'pri' : 'danger'), r.state === 'fuera' ? 'Recuperar' : 'Descartar');
    bOut.style.flex = '1';
    bOut.onclick = () => { r.state = r.state === 'fuera' ? (r.chosen ? 'ok' : 'nuevo') : 'fuera'; renderTicketReview(); };
    row.appendChild(bNew); row.appendChild(bOut);
    c.appendChild(row);

    if (r.state === 'fuera') c.style.opacity = '.45';
    box.appendChild(c);
  });
}

$('#btnTicketAdd').onclick = () => {
  let n = 0;
  ticketRows.forEach(r => { if (r.state !== 'fuera' && r.chosen) { addToPantry(r.chosen, true); n++; } });
  save(); closeSheet('#sheetTicket'); go('Pantry');
  toast(`${n} producto${n === 1 ? '' : 's'} en la despensa`);
};

/* ================= PLAN SEMANAL ================= */
let planDay = 0;

/* El objetivo del plan usa tu actividad MEDIA, no la de hoy: si hoy no has
   apuntado pasos ni entreno, el menú de la semana saldría 400 kcal corto. */
function planTarget() {
  const recent = Object.keys(P.days).sort().slice(-14)
    .filter(d => (P.days[d].steps || 0) > 0 || (P.days[d].workouts || []).length);
  let maint;
  if (recent.length >= 3) {
    maint = recent.reduce((s, d) => s + maintenanceFor(d), 0) / recent.length;
  } else {
    maint = maintenanceFor(todayISO());
    if (!(P.days[todayISO()] || {}).steps) maint += stepsKcal(7000) * (P.mode === 'dyn' ? 1 : 0);
  }
  const t = maint * (1 + goalAdjust());
  const m = macroTargets(t);
  return { kcal: t, p: m.p, c: m.c, f: m.f };
}

function renderPlan() {
  seg('#segMealsPerDay', String(P.mealsPerDay));
  const t = planTarget();
  $('#planTargetNote').innerHTML = `Objetivo diario: <b class="num">${r0(t.kcal)} kcal</b> · P${r0(t.p)} C${r0(t.c)} G${r0(t.f)} g`;

  const avail = pantryFoods();
  $('#planGaps').textContent = avail.length
    ? `${avail.length} productos disponibles en la despensa.`
    : 'La despensa está vacía o todo marcado como que falta. Escanea un ticket primero.';

  /* lista de la compra */
  const shop = $('#planShopping'); shop.innerHTML = '';
  const out = P.pantry.filter(x => !x.have);
  const gaps = P.plan ? P.plan.gaps : [];
  if (out.length || gaps.length) {
    const c = el('div', 'card');
    c.innerHTML = `<h2>Falta comprar<span class="side">${out.length + gaps.length}</span></h2>`;
    if (gaps.length) c.innerHTML += `<div class="tiny" style="margin-bottom:10px">Para cuadrar el menú te falta: <b>${gaps.map(esc).join(', ')}</b>.</div>`;
    if (out.length) {
      const l = el('div', 'list'); l.style.margin = '0';
      out.forEach(x => {
        const b = el('button', 'item');
        b.innerHTML = `<div class="grow"><div class="t">${esc(x.food.n)}</div></div><div class="tiny">marcar como comprado</div>`;
        b.onclick = () => { x.have = true; save(); renderPlan(); };
        l.appendChild(b);
      });
      c.appendChild(l);
    }
  ...
  if (!P.plan) {
    body.innerHTML = '<div class="list"><div class="empty">Todavía no hay menú.<br>Pulsa «Generar semana».</div></div>';
    return;
  }

  const tabs = el('div', 'seg'); tabs.style.marginBottom = '14px';
  DAYNAMES.forEach((n, i) => {
    const b = el('button', null, n);
    b.setAttribute('aria-pressed', i === planDay);
    b.onclick = () => { planDay = i; renderPlan(); };
    tabs.appendChild(b);
  });
  body.appendChild(tabs);

  const d = P.plan.days[planDay];
  const dev = (v, tg) => { const p = (v - tg) / tg * 100; return `${p >= 0 ? '+' : ''}${r0(p)}%`; };
  const st = el('div', 'card');
  st.innerHTML = `<h2>Total del día</h2><div class="stats">
    <div class="stat"><b class="num">${r0(d.totals.kcal)}</b><span>kcal</span><span>${dev(d.totals.kcal, t.kcal)}</span></div>
    <div class="stat"><b class="num">${r0(d.totals.p)}</b><span>proteína</span><span>${dev(d.totals.p, t.p)}</span></div>
    <div class="stat"><b class="num">${r0(d.totals.c)}</b><span>carbos</span><span>${dev(d.totals.c, t.c)}</span></div>
    <div class="stat"><b class="num">${r0(d.totals.f)}</b><span>grasa</span><span>${dev(d.totals.f, t.f)}</span></div></div>`;
  body.appendChild(st);

  d.meals.forEach((m, mi) => {
    body.appendChild(el('div', 'grouphead',
      `<span>${esc(m.name)}</span><span class="num">${r0(m.totals.kcal)} kcal</span>`));
    const l = el('div', 'list');
    m.items.forEach((it, ii) => {
      const mac = PLAN.macrosOf(it.food, it.grams, it.cook);
      const opts = cookOptions(it.food);
      const ck = opts[it.cook];
      const b = el('button', 'item');
      b.innerHTML = `<div class="grow"><div class="t">${esc(it.food.n)}</div>
        <div class="s">${r0(it.grams)} g${Object.keys(opts).length > 1 ? ' · ' + esc((ck || {}).n || '').toLowerCase() : ''}${mac.oilG >= 0.5 ? ' · +' + r1(mac.oilG) + ' g aceite' : ''}</div></div>
        <div class="k num">${r0(mac.kcal)}<em>P${r0(mac.p)} C${r0(mac.c)} G${r0(mac.f)}</em></div>`;
      b.onclick = () => openPlanItem(planDay, mi, ii);
      l.appendChild(b);
    });
    const addB = el('button', 'item');
    addB.innerHTML = '<div class="grow"><div class="t" style="color:var(--link)">Añadir alimento a esta comida</div></div>';
    addB.onclick = () => openSearch({
      pick: f => {
        m.items.push({ food: f, grams: 100, cook: PLAN.defCook(f), grp: 'C' });
        recalcPlanDay(planDay); save(); closeSheet('#sheetSearch'); renderPlan();
      }
    });
    l.appendChild(addB);
    body.appendChild(l);
  ...
  const row = el('div', 'btnrow'); row.style.margin = '14px 0';
  const b1 = el('button', 'btn', 'Otro menú para este día');
  b1.onclick = () => {
    const pools = PLAN.poolsFrom(pantryFoods());
    P.plan.days[planDay] = PLAN.buildDay(planTarget(), pools, P.plan.mealsPerDay, Date.now() % 99991);
    save(); renderPlan();
  };
  const b2 = el('button', 'btn pri', 'Enviar al diario');
  b2.onclick = () => sendPlanToDiary(planDay);
  row.appendChild(b1); row.appendChild(b2);
  body.appendChild(row);
}

function recalcPlanDay(di) {
  const d = P.plan.days[di];
  d.meals.forEach(m => m.totals = PLAN.sumItems(m.items));
  d.totals = PLAN.sumItems(d.meals.flatMap(m => m.items));
}

$$('#segMealsPerDay button').forEach(b => b.onclick = () => {
  P.mealsPerDay = +b.dataset.v; save(); renderPlan();
});

$('#btnGenPlan').onclick = () => {
  const avail = pantryFoods();
  if (avail.length < 4) return toast('Marca al menos 4 productos disponibles');
  P.plan = PLAN.buildWeek(planTarget(), avail, P.mealsPerDay, Date.now() % 99991);
  planDay = 0; save(); renderPlan();
  toast(P.plan.gaps.length ? 'Menú hecho, pero falta algo' : 'Menú de la semana listo');
};

function sendPlanToDiary(di) {
  const d = P.plan.days[di];
  const target = addDays(mondayOf(todayISO()), di);
  if (!confirm(`¿Copiar este menú al ${niceDate(target).toLowerCase()}?`)) return;
  const dd = (P.days[target] ||= { meals: [], steps: 0, workouts: [], weight: null });
  d.meals.forEach(m => m.items.forEach(it => {
    dd.meals.push({
      id: uid(), meal: m.name,
      food: { id: it.food.id, n: it.food.n, cl: it.food.cl || 'simple', kcal: it.food.kcal, p: it.food.p, c: it.food.c, f: it.food.f, fib: it.food.fib || 0, units: it.food.units || [], barcode: it.food.barcode },
      grams: it.grams, cook: it.cook, weighedCooked: false, oil: null
    });
  }));
  save(); viewDate = target; go('Today'); toast('Menú copiado al diario');
}

/* ---------- Ajustar un alimento del menú ---------- */
let piRef = null;
function openPlanItem(di, mi, ii) {
  piRef = { di, mi, ii };
  const it = P.plan.days[di].meals[mi].items[ii];
  $('#piTitle').textContent = it.food.n;
  $('#piGrams').value = r0(it.grams);
  const opts = cookOptions(it.food);
  const sel = $('#piCook'); sel.innerHTML = '';
  Object.entries(opts).forEach(([k, v]) => sel.appendChild(new Option(v.n, k)));
  sel.value = it.cook;
  syncPlanItem();
  openSheet('#sheetPlanItem');
}
function planItemRef() { return P.plan.days[piRef.di].meals[piRef.mi].items[piRef.ii]; }
function syncPlanItem() {
  const it = planItemRef();
  const m = PLAN.macrosOf(it.food, +$('#piGrams').value || 0, $('#piCook').value);
  $('#piPreview').innerHTML = `
    <div class="stat"><b class="num">${r0(m.kcal)}</b><span>kcal</span></div>
    <div class="stat"><b class="num">${r1(m.p)}</b><span>proteína</span><span>g</span></div>
    <div class="stat"><b class="num">${r1(m.c)}</b><span>carbos</span><span>g</span></div>
...
$('#piGrams').addEventListener('input', syncPlanItem);
$('#piCook').addEventListener('change', syncPlanItem);
$('#btnPiSave').onclick = () => {
  const it = planItemRef();
  it.grams = Math.max(1, +$('#piGrams').value || 0);
  it.cook = $('#piCook').value;
  recalcPlanDay(piRef.di); save(); closeSheet('#sheetPlanItem'); renderPlan();
};
$('#btnPiDelete').onclick = () => {
  const m = P.plan.days[piRef.di].meals[piRef.mi];
  m.items.splice(piRef.ii, 1);
  recalcPlanDay(piRef.di); save(); closeSheet('#sheetPlanItem'); renderPlan();
};
$('#btnPiSwap').onclick = () => openSearch({
  pick: f => {
    const it = planItemRef();
    it.food = f; it.cook = PLAN.defCook(f);
    recalcPlanDay(piRef.di); save();
    closeSheet('#sheetSearch'); closeSheet('#sheetPlanItem'); renderPlan();
  }
});
